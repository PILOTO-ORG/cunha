...
import { useCriarOrcamento } from '../hooks/useOrcamentosReservas';
...
interface ReservaCheckoutFormProps {
  ...
  eventoInicio?: string; // data de início do evento (pré-definida)
  eventoFim?: string; // data de fim do evento (pré-definida)
  mode?: 'view' | 'edit'; // modo de visualização ou edição
  /**
   * Optional custom save handler. If provided, the form will call this
   * function with the prepared payload (including items) instead of
   * calling the internal OrcamentoService/creation flow. This is useful
   * when reusing the checkout UI to save a RESERVA instead of an ORÇAMENTO.
   *
   * Signature: async ({ idOrcamento, data, items }) => any
   */
...
const ReservaCheckoutForm: React.FC<ReservaCheckoutFormProps> = ({
  ...
  const [orcamentoCriadoId, setOrcamentoCriadoId] = useState<number | null>(null);

  // Hook para criar orçamento
  const criarOrcamentoMutation = useCriarOrcamento();
  ...
  const [formData, setFormData] = useState(() => {
    // Função para garantir que datas tenham horário
    const formatWithTime = (dateStr: string) => {
      if (!dateStr) return '';
      if (dateStr.includes('T')) return dateStr;
      return `${dateStr}T09:00`; // horário padrão 9:00
    };

    return {
      id_cliente: 0,
      id_local: 0,
      evento_inicio: formatWithTime(eventoInicio || ''),
      evento_fim: formatWithTime(eventoFim || ''),
      evento_entrega: '',
      evento_recolha: '',
      observacoes: '',
      forma_pagamento: '',
  ...
  // Reaproveitar datas da página anterior via URL params se não foram passadas como props
  React.useEffect(() => {
    if (!eventoInicio || !eventoFim) {
      ...
      if (inicioParam || fimParam) {
        // Adicionar horário padrão se não estiver presente
        const formatWithTime = (dateStr: string) => {
          if (dateStr.includes('T')) return dateStr;
          return `${dateStr}T09:00`; // horário padrão 9:00
        };
        ...
        setFormData(prev => ({
          ...prev,
          evento_inicio: inicioParam ? formatWithTime(inicioParam) : prev.evento_inicio,
          evento_fim: fimParam ? formatWithTime(fimParam) : prev.evento_fim
        }));
      }
    }
  }, [eventoInicio, eventoFim]);

  // Calcular automaticamente as datas de entrega e recolha baseadas nas datas do evento
  React.useEffect(() => {
    if (formData.evento_inicio && formData.evento_fim) {
      const inicio = new Date(formData.evento_inicio);
      const fim = new Date(formData.evento_fim);
      
      // entrega = data_inicio - 1 dia (mantém o horário)
      const entrega = new Date(inicio);
      entrega.setDate(entrega.getDate() - 1);
      
      // recolha = data_fim + 1 dia (mantém o horário)
      const recolha = new Date(fim);
      recolha.setDate(recolha.getDate() + 1);
      ...
      setFormData(prev => ({
        ...prev,
        evento_entrega: formatDateTime(entrega),
        evento_recolha: formatDateTime(recolha)
      }));
    }
  }, [formData.evento_inicio, formData.evento_fim]);
  ...
  React.useEffect(() => {
    const fetchOrcamento = async () => {
      ...
      try {
        ...
        const orcamento = await OrcamentoService.buscarOrcamento(idOrcamento);
        const itensReserva = await OrcamentoService.listarItensOrcamento(idOrcamento);

        setFormData(prev => ({
          ...prev,
          id_cliente: orcamento.id_cliente || 0,
          id_local: orcamento.id_local || 0,
          evento_inicio: formatDateTimeForInput(orcamento.evento_inicio) || '',
          evento_fim: formatDateTimeForInput(orcamento.evento_fim) || '',
          evento_entrega: formatDateTimeForInput(orcamento.evento_entrega) || '',
          evento_recolha: formatDateTimeForInput(orcamento.evento_recolha) || '',
  ...
  const calcularDiasPeriodo = () => {
    if (!formData.evento_inicio || !formData.evento_fim) return 1;
    const dias = Math.max(1, Math.ceil((new Date(formData.evento_fim).getTime() - new Date(formData.evento_inicio).getTime()) / (1000 * 60 * 60 * 24)));
    return dias;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let orcamento;
      // If a custom save handler is provided (e.g. to save a RESERVA instead
      // of an ORÇAMENTO), call it and bail out of the default orcamento flow.
      if (onSave) {
        const payload: any = {
          id_cliente: Number(formData.id_cliente),
          id_local: formData.id_local > 0 ? Number(formData.id_local) : undefined,
          evento_inicio: formData.evento_inicio,
          evento_fim: formData.evento_fim,
          evento_entrega: formData.evento_entrega,
          evento_recolha: formData.evento_recolha,
        ...
        const result = await onSave({ idOrcamento, data: payload, items: itemsData });
        // Let the parent component handle navigation/state after save
        onSuccess(result);
        return;
      }
      ...
      if (idOrcamento) {
        ...
        // @ts-ignore
        updateData.itens = itemsData;
        orcamento = await OrcamentoService.atualizarOrcamento(idOrcamento, updateData);
      } else {
        // Criar orçamento novo usando o hook
        ...
        // Usar o hook para criação que já invalida as queries
        const response = await criarOrcamentoMutation.mutateAsync(orcamentoData);
        
        // Extrair o ID do orçamento criado da resposta
        // A resposta tem a estrutura: { success: true, message: "...", data: {...}, pdfUrl: "..." }
        let orcamentoId = null;
        
        if ((response as any).data?.id_orcamento) {
          orcamentoId = (response as any).data.id_orcamento;
        } else if ((response as any).id_orcamento) {
          orcamentoId = (response as any).id_orcamento;
        } else if ((response as any).id) {
          orcamentoId = (response as any).id;
        } else if ((response as any).data?.id) {
          orcamentoId = (response as any).data.id;
        }
        
        console.log('ID do orçamento extraído:', orcamentoId);
        
        // Mostrar modal de sucesso em vez de navegar diretamente
        setOrcamentoCriadoId(orcamentoId);
  ...
  const handleTrocarStatus = async (novoStatus: string) => {
    if (!idOrcamento) return;
    
    try {
      setIsLoading(true);
      await OrcamentoService.atualizarOrcamento(idOrcamento, {
      ...
      setError(error.response?.data?.error || 'Erro ao trocar status. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGerarPDF = async () => {
    ...
    try {
      await ReservaService.gerarPDFOrcamento(orcamentoCriadoId);
    } catch (error) {
  ...
  const handleGerarAssinatura = async () => {
    ...
    try {
      const result = await ReservaService.gerarLinkAssinatura(orcamentoCriadoId);
      return result;
    } catch (error) {
  ...
  const handleFecharModal = () => {
    ...
    setOrcamentoCriadoId(null);
    // Pequeno delay antes de navegar para garantir que o modal seja fechado
    setTimeout(() => {
      navigate('/orcamentos');
    }, 300);
  };
    ...
    <div className="w-full bg-white rounded-xl shadow-2xl overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Início do Evento <span className="text-red-500">*</span>
                </label>
                <Input
                  type="datetime-local"
                  name="evento_inicio"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Fim do evento <span className="text-red-500">*</span>
                </label>
                <Input
                  type="datetime-local"
                  name="evento_fim"
                  value={formData.evento_fim}
                  onChange={handleChange}
                  className="w-full border-2 focus:ring-2 focus:ring-purple-500"
                  required
                />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <Input
                  type="datetime-local"
                  name="evento_entrega"
                  value={formData.evento_entrega}
                  onChange={handleChange}
                  className="w-full border-2 focus:ring-2 focus:ring-purple-500"
                />
                <p className="text-xs text-gray-500 mt-1">Data para entrega dos produtos</p>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Data de Recolha 
                ...
                <Input
                  type="datetime-local"
                  name="evento_recolha"
                  value={formData.evento_recolha}
                  onChange={handleChange}
                  className="w-full border-2 focus:ring-2 focus:ring-purple-500"
                />
                <p className="text-xs text-gray-500 mt-1">Data para recolha dos produtos</p>
              </div>
            </div>
              ...
              <div className="mt-4 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                <div className="flex items-center text-sm text-purple-900">
                  ...
                  <span className="font-medium">
                    Período: {calcularDiasPeriodo()} dia(s) de locação
                  </span>
                </div>
              </div>
            )}
                ...
                {localSelecionado && !editandoLocal && (
                  <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-sm font-bold text-green-900 flex items-center">
                        <svg className="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4zM9 12a1 1 0 112 0 1 1 0 01-2 0z" clipRule="evenodd" />
                        </svg>
              ...
              <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-5 py-3">
                <h3 className="text-base font-bold text-white flex items-center">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9v2.828l-1.293-1.293a1 1 0 00-1.414 1.414L9 12l-2.293 2.293a1 1 0 001.414 1.414L9 14.414V17a2 2 0 002 2z" />
                  </svg>
        ...
        <div className="sticky bottom-0 bg-white border-t-4 border-gray-200 mt-8 -mx-6 sm:-mx-8 px-6 sm:px-8 py-4 shadow-2xl">
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              {mode === 'edit' && idOrcamento && (
                <button
                  type="button"
                  onClick={() => handleTrocarStatus('Cancelado')}
                  disabled={isLoading || formData.status === 'Cancelado'}
                  className="inline-flex items-center justify-center font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-200 border text-red-600 border-red-300 bg-white hover:bg-red-50 focus:ring-red-500 px-4 py-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true" className="w-5 h-5 mr-2">
              ...
              {mode === 'view' && (
                <Button
                  type="button"
                  onClick={() => {/* TODO: implementar mudança para modo edit */}}
                  className="flex-1 sm:flex-none px-8 py-3 text-base font-semibold bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800"
                >
      ...
      {showSucessoModal && (
        <OrcamentoSucessoModal
          orcamentoId={orcamentoCriadoId || 1}
